...
from tfnlp.data import Batcher
from tfnlp.data import TokenBatcher
from tfnlp.data import Vocabulary 
from tfnlp.data import UnicodeCharsVocabulary
    ...
    def __init__(
        ...
        if not use_character_inputs:
            if embedding_weight_file is None:
                raise ValueError(
                    "embedding_weight_file is required input with not use_character_inputs"
                )
...
class BiLanguageModelGraph(object):
    '''
    ...
    def __init__(self, options, weight_file, ids_placeholder,
        ...
        def custom_getter(getter, name, *args, **kwargs):
            ...
            kwargs['initializer'] = BiLanguageModelGraph.pretrained_initializer(
    ...
    @staticmethod
    def pretrained_initializer(varname, \
            weight_file, embedding_weight_file=None):
        '''
    ...
    def _build_lstms(self):
        ...
        if use_skip_connections:
            ...
            print("lstm not using skip connections ......")
        ...
        for direction in ['forward', 'backward']:
            ...
            for i in range(n_lstm_layers):
                ...
                init_states = [
                    tf.Variable( tf.zeros([self._max_batch_size, dim]), trainable=False)
...
def dump_token_embeddings(vocab_file, options_file, weight_file, outfile):
    ...
    constructing a BiLanguageModel.
    '''
    with open(options_file, 'r') as fin:
        options = json.load(fin)
    ...
    model = BiLanguageModel(options_file, weight_file)
    ...
    with tf.Session(config=config) as sess:
        ...
        for k in range(n_tokens):
            ...
            char_ids = batcher.batch_sents([[token]])[0, 1, :].reshape(
                1, 1, -1)
...
def dump_bilm_embeddings(vocab_file, dataset_file, options_file,
                         weight_file, outfile, emb_file = None):
    ...
    if "char_cnn" in options:
        ...
        model = BiLanguageModel(options_file, weight_file, \
                use_character_inputs=use_char, embedding_weight_file=emb_file)
    ...
    with tf.Session(config=config) as sess:
        ...
        with open(dataset_file, 'r') as fin, h5py.File(outfile, 'w') as fout:
            for line in fin:
                ...
                char_ids = batcher.batch_sents([sentence])
                embeddings = sess.run(
