...
Brief:  bidirectional language models.
"""
...
from tfnlp.nets.elmo.data_lm import InvalidNumberOfCharacters
...
class LanguageModel(object):
    ...
    def __init__(self, options, is_training):
        ...
        ## model var
        ## 词emb weight， 正反 tok + tensor
        self.token_ids = None
        self.token_ids_reverse = None
        self.embedding = None
        self.embedding_reverse = None
        self.embedding_weights = None

        #char embedings
        self.tokens_characters = None
        self.tokens_characters_reverse = None
        self.char_embedding = None
        self.char_embedding_reverse = None
        self.token_embedding_layers = []
    ...
    def _build_word_embeddings(self):
        ...
        with tf.device("/cpu:0"):
            #通过word2vec 进行初始化
            emb_file = self.options.get("word_emb_file", "")
            if self.is_training and emb_file != "":
                w2v_model = gensim.models.KeyedVectors.load_word2vec_format(fname = emb_file)
                tmp_embd = np.zeros((n_tokens_vocab, projection_dim), dtype=DTYPE)
                words = w2v_model.vocab
                for i, word in enumerate(words):
                    tmp_embd[i+3][:] = w2v_model[word]
        ...
        def make_convolutions(inp, reuse):
            with tf.variable_scope('CNN', reuse=reuse) as scope:
                ...
                for i, (width, num) in enumerate(filters):
                    ...
                    #get var w and b
                    w = tf.get_variable(
                        "W_cnn_%s" % i, [1, width, char_embed_dim, num], initializer = w_init, dtype = DTYPE)
                    
                    b = tf.get_variable(
                        "b_cnn_%s" % i, [num], initializer = tf.constant_initializer(0.0), dtype = DTYPE)
        ...
        if use_proj:
            ...
            with tf.variable_scope('CNN_proj') as scope:
                    W_proj_cnn = tf.get_variable(
                        ...
                        initializer=tf.random_normal_initializer(mean=0.0, stddev=np.sqrt(1.0 / n_filters)),
                        dtype = DTYPE)
    ...
    def _build(self):
        ...
        # now the LSTMs these will collect the initial states for the forward
        ...
        if use_skip_connections:
            print("lstm using skip connections ......")
        ...
        with tf.variable_scope('softmax'), tf.device('/cpu:0'):
            ...
            if not self.share_embedding_softmax:
                self.softmax_W = tf.get_variable(
                    ...
                    dtype=DTYPE, initializer = softmax_init
                )
    ...
    with tf.Session(config=tf.ConfigProto( allow_soft_placement=True)) as sess:
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            if batch_no == n_batches_total:
                # done training!
                emb_file = tf_save_dir + "/vocab_embedding.hdf5"
...
def test(options, ckpt_file, data, batch_size=256):
    ...
    print("finsihed! avg_perplexity = %s" % np.exp(avg_loss))
    return np.exp(avg_loss)



# tensorflow 模型weight dump 成hdf5格式
def dump_weights(tf_save_dir, outfile):
    ...
    options, ckpt_file = bilm_util.load_options_latest_checkpoint(tf_save_dir)
